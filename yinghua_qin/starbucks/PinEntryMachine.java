...
//    private int pinCount=0 ;
 ...
 //       this.pinCount = 1 ;
        if ( this.auth != null )
        	 this.auth.authFailEvent(2);  //final: fail valid then input one pin, clear invalid pin message
        this.state = pin1 ;
        if ( digit != null )
            this.d1 = digit ;
        else {
...
//        this.pinCount = 4 ;
        ...
        if ( digit != null )
        {
            ...
            else
            {
                ...
                this.authenticated = false ;notifyObserver() ;
                setStateNoPinDigits() ;
            }
        }
    }
    ...
    {
        if ( this.auth != null )
            if(this.authenticated) this.auth.authEvent() ;
            else this.auth.authFailEvent(1);  //final: fail valid, show invalid pin message
    }
